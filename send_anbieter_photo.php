...
echo '<br>';
echo $_FILES['anbieter_photo']['name'];
echo '<br>';
echo $_FILES["anbieter_photo"]["size"];
echo '<br>';
echo $_FILES["anbieter_photo"]["tmp_name"];
echo '<br>';
...
if (isset($_POST["submit"])) {
    $check = getimagesize($_FILES["anbieter_photo"]["tmp_name"]);
    if ($check !== false) {
        echo '<br>';
        ...
        window.location = 'http://172.16.32.201/~lauk/Buchungsplattform/anbieter_profilbild_form.php?AnbieterID=$AnbieterID'";
        echo "</script>";
        $uploadOk = 0;
    }
}
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES['anbieter_photo']['tmp_name'], $target_file)) {
        updatePhoto($conn, $target_file, $AnbieterID);
        echo "<script type='text/javascript'>";
        echo "alert('Das Foto wurde hochgeladen!');
        window.location = 'http://172.16.32.201/~lauk/Buchungsplattform/anbieter_unterkunftsfotos_form.php?AnbieterID=$AnbieterID'";
        echo "</script>";
    } else {
