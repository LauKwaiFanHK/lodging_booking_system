...
if (isset($_POST["submit"])) {
    ...
    if ($check !== false) {
        ...
        window.location = 'http://172.16.32.201/~lauk/Buchungsplattform/unterkunftsanbieter_registrieren.php?AnbieterID=$AnbieterID'";
        echo "</script>";
        $uploadOk = 0;
    }
}
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES['unterkunft_photo']['tmp_name'], $target_file)) {
        updateUnterkunftPhoto1($conn, $target_file, $AnbieterID);
        echo "<script type='text/javascript'>";
        echo "alert('Das Foto wurde hochgeladen!');
    window.location = 'http://172.16.32.201/~lauk/Buchungsplattform/anbieter_unterkunftsfotos_form2.php?AnbieterID=$AnbieterID'";
        echo "</script>";
    } else {
